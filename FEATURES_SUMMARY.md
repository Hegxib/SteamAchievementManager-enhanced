...
## 🎯 All Implemented Features (V1.4.0 Complete!)

### Phase 6: System Integration & Stability (V1.4.0) ✅
**Live Language Sync**
- Changing language in `SAM.Picker` instantly updates all open `SAM.Game` windows.
- Uses `FileSystemWatcher` to monitor `%APPDATA%\SAM\language.txt`.

**Windows Auto-Start**
- Toggle button in the toolbar to enable/disable auto-start.
- Manages `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run` registry key.

**Enter Key Support**
- Pressing `Enter` in the game list launches all selected games.
- Works with multi-selection.

**Universal Compatibility**
- UTF-8 (No BOM) encoding for all language files.
- ASCII-based symbols (e.g., `[Lang]`, `[?]`) instead of emojis for perfect display on all Windows versions.
